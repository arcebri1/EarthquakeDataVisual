...
// Create an object so only one base layer can be shown at a time
let baseMaps = {
    Satellite: satellite,
    Grayscale: light,
    Outdoors: outdoors
...
// Initialize all of the overlay LayerGroups we'll be using
let layers = {
...
d3.json(plates_url, function (plates_data) {
    ...
    L.geoJSON(plates_data, {
        style: function () {
            return { color: "red", fillOpacity: .75 }
        }

    }).addTo(myMap)

    // let plates = plates_data.features
    // // console.log(plates)

    // for (let i=0; i < plates.length; i++) {

    // let plates_coordinates = plates[i].geometry.coordinates
    // console.log([plates_coordinates[0]])

    // let coordinates = plates_coordinates[0]
    // console.log(coordinates)


    // let some = "https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json"
    // d3.json(some, function(one1){
    //     console.log(one1)
    // })
